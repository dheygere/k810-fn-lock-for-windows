...
This program sets the function keys for the Logitech K810 bluetooth keyboard on Windows. 
...
{
    int seq_len = 7;
    const unsigned char k810_seq_fkeys_on[] =  {0x10, 0xFF, 0x06, 0x15, 0x00, 0x00, 0x00};
    const unsigned char k810_seq_fkeys_off[] = {0x10, 0xFF, 0x06, 0x15, 0x01, 0x00, 0x00};
    ...
    devs = hid_enumerate(k810_vid, k810_pid);
    cur_dev = devs;
    ...
    while (cur_dev)
    {
        if (cur_dev->usage == TARGET_USAGE && cur_dev->usage_page == TARGET_USAGE_PAGE)
        {
            ...
            res = hid_write(handle, k810_seq_fkeys_off, seq_len);
#else
            printf("Set function keys as default");
            res = hid_write(handle, k810_seq_fkeys_on, seq_len);
#endif
